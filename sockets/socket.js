"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
exports.__esModule = true;
var socketIo = require("socket.io");
var request = require("request");
var Rooms_1 = require("../models/Rooms");
var errorCodes_1 = require("../server/tools/errors/errorCodes");
var NODE_ENV = process.env.NODE_ENV;
var urlController;
console.log(NODE_ENV);
if (NODE_ENV === 'development')
    urlController = 'http://localhost:8083/api/prostagmaApi/';
else
    urlController = 'http://prostagma.fr/api/prostagmaApi/';
function requestFunction(url, object) {
    return new Promise(function (resolve, reject) {
        request.post({
            url: urlController + url,
            qs: object
        }, function (err, httpResponse, body) {
            // @ts-ignore
            if (errorCodes_1.ErrorsText[body * 1] && body * 1 === errorCodes_1.ErrorsText[body * 1].statusCode) {
                // @ts-ignore
                reject(errorCodes_1.ErrorsText[httpResponse.statusCode]);
                return false;
            }
            var chatRequest = JSON.parse(body);
            resolve(chatRequest);
        });
    });
}
exports.sockets = function (server) {
    var io = socketIo.listen(server);
    var chatSocketHandler = io.of('/chat');
    chatSocketHandler.on('connection', function (socket) {
        socket.on('pingConnection', function (user) { return __awaiter(void 0, void 0, void 0, function () {
            var obj;
            return __generator(this, function (_a) {
                console.log('user');
                obj = {
                    userId: user._id,
                    socketId: socket.id
                };
                requestFunction('db/updateUserSocketId', obj)
                    .then(function (u) {
                    chatSocketHandler.emit('connectedUser', u);
                    chatSocketHandler.emit('userToPush', u);
                    requestFunction('db/getRoomsByUserId', u)
                        .then(function (rooms) {
                        rooms.forEach(function (room) {
                            chatSocketHandler.sockets[u.socketId].join(room.userIds);
                            chatSocketHandler.to(room.userIds).emit('userReconnected', u);
                        });
                    })["catch"](function (e) {
                        console.log(e);
                    });
                });
                return [2 /*return*/];
            });
        }); });
        socket.on('joinRoom', function (socketIds) { return __awaiter(void 0, void 0, void 0, function () {
            var room, roomId, arraySocketIds, obj;
            return __generator(this, function (_a) {
                room = new Rooms_1.Rooms();
                roomId = socketIds;
                arraySocketIds = socketIds.split('&');
                obj = {
                    socketIds: arraySocketIds,
                    roomID: roomId
                };
                requestFunction('db/getUserBySocketId', obj)
                    .then(function (users) {
                    if (users !== '0') {
                        var userIds_1 = '';
                        var i = 0;
                        while (i < users.length) {
                            userIds_1 += users[i]._id;
                            if (i + 1 !== users.length) {
                                userIds_1 += '#';
                            }
                            i++;
                        }
                        arraySocketIds.forEach(function (socketId) {
                            chatSocketHandler.to(socketId).emit('createChat', 'You have joined');
                            chatSocketHandler.sockets[socketId].join(userIds_1);
                        });
                        console.log(userIds_1);
                        room.roomId = roomId;
                        room.users = users;
                        room.userIds = userIds_1;
                        var saveRoom = requestFunction('/db/saveRoom', room)["catch"](function (e) {
                            console.log('wtf');
                            console.log(e);
                        });
                    }
                });
                return [2 /*return*/];
            });
        }); });
        socket.on('disconnect', function (infos) { return __awaiter(void 0, void 0, void 0, function () {
            var obj;
            return __generator(this, function (_a) {
                obj = { socketIds: [socket.id] };
                requestFunction('db/getUserBySocketId', obj)
                    .then(function (user) {
                    if (user !== 'No user found.') {
                        var userToDisconnect = { userID: user[0]._id };
                        requestFunction('db/getRoomsByUserId', user[0])
                            .then(function (rooms) {
                            rooms.forEach(function (room) {
                                chatSocketHandler.to(room.userIds).emit('userDisconnection', user[0]);
                            });
                        });
                        requestFunction('db/signOutUser', userToDisconnect)
                            .then(function (u) {
                            console.log(u.username + ' has disconnected');
                        })["catch"](function (e) {
                            console.log('Error while disconnecting user');
                            console.log(e);
                        });
                    }
                })["catch"](function (e) {
                    console.log(e);
                });
                return [2 /*return*/];
            });
        }); });
        socket.on('simpleMessage', function (response) {
            chatSocketHandler.to("" + response.chatRequest.socketId).emit('newConv', response.sender);
            chatSocketHandler.to("" + response.chatRequest.socketId).emit('msgReceived', response);
        });
    });
};
